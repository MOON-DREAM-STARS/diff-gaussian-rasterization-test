	...
	int num_rendered)
{
	...
	if (keys_unsorted != nullptr && values_unsorted != nullptr && num_rendered > 0)
	{
		...
		cudaMemcpy(h_keys, keys_unsorted, num_rendered * sizeof(uint64_t), cudaMemcpyDeviceToHost);
		cudaMemcpy(h_values, values_unsorted, num_rendered * sizeof(uint32_t), cudaMemcpyDeviceToHost);
